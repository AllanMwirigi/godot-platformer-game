...
#this is an inbuilt func which when included in node, will be called in every frame i.e. ~60/s
...
#func _physics_process(delta: float) -> void:
	# delta is a value given by engine on every tick of this function
		...
		# ensures consistency based on the speed of user's device, makes game frame-rate independent
	
	# _velocity.y = min(velocity.y, speed.y)  #caps the max fall velocity to speed
